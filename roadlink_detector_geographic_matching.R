...
### check Francisco's method
# mark distance > 100ft (1)
...
for (i in 1:nrow(sidefire_2025)) {
  if (sidefire_2025$FUNCL[i] != roadlink_2026$FUNCL[which(roadlink_2026$ID == sidefire_2025$nearlink_id_26[i])]) {
...
# check if NB, SB, EB, WB match
sidefire_2025$dir_notmatch_26 = 0
for (i in 1:nrow(sidefire_2025)) {
  # count name contains NB but road link name does not
  if (isTRUE(grepl("NB", sidefire_2025$LINKNAME[i]))) {
    if (isFALSE(grepl('NB', sidefire_2025$nearlink_name_26[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  } 
  # count name contains SB but road link name does not
  else if (isTRUE(grepl("SB", sidefire_2025$LINKNAME[i]))) {
    if (isFALSE(grepl('SB', sidefire_2025$nearlink_name_26[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  } 
  # count name contains EB but road link name does not
  else if (isTRUE(grepl("EB", sidefire_2025$LINKNAME[i]))) {
    if (isFALSE(grepl('EB', sidefire_2025$nearlink_name_26[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  }
  # count name contains WB but road link name does not
  else if (isTRUE(grepl("WB", sidefire_2025$LINKNAME[i]))) {
    if (isFALSE(grepl('WB', sidefire_2025$nearlink_name_26[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  } 
}
...
for (i in 1:nrow(sidefire_2025)) {
  # count name does not contain NB but road link name has
  if (isTRUE(grepl("NB", sidefire_2025$nearlink_name_26[i]))) {
    if (isFALSE(grepl('NB', sidefire_2025$LINKNAME[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  } 
  # count name does not contain SB but road link name has
  else if (isTRUE(grepl("SB", sidefire_2025$nearlink_name_26[i]))) {
    if (isFALSE(grepl('SB', sidefire_2025$LINKNAME[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  }
  # count name does not contain EB but road link name has
  else if (isTRUE(grepl("EB", sidefire_2025$nearlink_name_26[i]))) {
    if (isFALSE(grepl('EB', sidefire_2025$LINKNAME[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  }
  # count name does not contain WB but road link name has
  else if (isTRUE(grepl("WB", sidefire_2025$nearlink_name_26[i]))) {
    if (isFALSE(grepl('WB', sidefire_2025$LINKNAME[i]))) {
      sidefire_2025$dir_notmatch_26[i] = 1
    }
  }
}
...
# manual checked, all of those SE, SW, NE, NW are correct
length(which(sidefire_2025$dir_notmatch_26 == 1))

# mark ramp not match (1)
sidefire_2025$ramp_notmatch_26 = 0
for (i in 1:nrow(sidefire_2025)) {
  # if count link is ramp while road link is not ramp
  if (isTRUE(grepl('Ramp', sidefire_2025$LINKNAME[i])) | isTRUE(grepl('ramp', sidefire_2025$LINKNAME[i]))) {
    if (isFALSE(grepl('RAMP', sidefire_2025$nearlink_name_26[i]))) {
      sidefire_2025$ramp_notmatch_26[i] = 1
    }
  }
  # if road link is ramp while count link is not ramp
  if (isTRUE(grepl('RAMP', sidefire_2025$nearlink_name_26[i]))) {
    if (isFALSE(grepl('Ramp', sidefire_2025$LINKNAME[i])) & isFALSE(grepl('ramp', sidefire_2025$LINKNAME[i]))) {
      sidefire_2025$ramp_notmatch_26[i] = 1
    }
  }
}
length(which(sidefire_2025$ramp_notmatch_26 == 1))
...
length(which(sidefire_2025$name_strictnotmatch_26 == 1))
