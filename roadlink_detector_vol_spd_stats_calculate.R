...
for (i in 1:nrow(sidefire_vol_spd_2022_am)) {
  ...
  spd_lowerff = which(c(t(link_i[,seq(139,149)])) < sidefire_vol_spd_2022_am$spdff[i])
  ...
  volarray_i = allNA_returnNA(t(link_i[,seq(46,56)])[spd_lowerff]) #,t(link_i[spd_lowerff,c(80,94)])))
  spdarray_i = allNA_returnNA(t(link_i[,seq(139,149)])[spd_lowerff]) #,t(link_i[spd_lowerff,c(173,187)])))
...
for (i in 1:nrow(sidefire_vol_spd_2022_pm)) {
  ...
  # find rows in link_i that are lower than spdff. if all > spdff, skip to next 
  spd_lowerff = which(c(t(link_i[,seq(139,149)])) < sidefire_vol_spd_2022_pm$spdff[i])
  if (length(spd_lowerff) == 0) {
    print(paste('All speeds higher than ff at sensor ID', link_i$LinkID[1]))
    next
  }
